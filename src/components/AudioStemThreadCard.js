...
export const AudioStemThreadCard = ({ thread }) => {
  ...
  const calculateAction = (worker) => {
    if (thread.solution && thread.solution.proposedBy == worker) {
      return ["winner", thread.solution.stems.length];
    }
    ...
    if (
      ...
      for (const validation of thread.validations) {
        if (validation.validator == worker)
          return ["Verified", validation.stems?.length || 0];
      }
    }
  };
    ...
    >
      <Grid item xs={12}>
        <Box display={"flex"} alignItems={"center"}>
          <Typography variant="body">From Stem</Typography>
          {!thread.completed ? (
