...
import AudioStemTaskCard from "./AudioStemTaskCard";

export const MyAudioStemTasks = () => {
  const { tasks, loading } = useSelector((state) => state.audioStem);
          ...
          {myTasks.length == 0 ? (
            ...
            <>
              {myTasks.map((task, index) => (
                <AudioStemTaskCard task={task} key={index} explorer={false} />
              ))}
            </>
          )}
        </>
