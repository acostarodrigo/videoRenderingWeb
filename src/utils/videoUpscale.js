...
import {
  ...
  setLoading,
  setTasks,
} from "state/videoUpscale";

import { create } from "ipfs-http-client";
import { saveAs } from "file-saver";

import { VideoUpscalerStargateClient } from "../videoUpscaleClient/dist/stargateClient";
import JSZip from "jszip";
export const getVideoUpscaleTasks = async () => {
  try {
    store.dispatch(setLoading(true));
    const cosmosClient = await VideoUpscalerStargateClient.connect(
      process.env.RPC_URL
    );
    const queryClient = cosmosClient.videoUpscalerQueryClient.videoUpscaler;
    const result = [];
    let totalAverage = 0;
    let amount = 0;
    for (let i = 1; i < 100; i++) {
      const task = await queryClient.GetVideoUpscalerTask(i.toString());
      if (task?.taskId) {
...
export const getWorkerLogs = async (worker, threadId) => {
  ...
  const cosmos = await VideoUpscalerStargateClient.connect(url);
  const query = cosmos.VideoUpscalerStargateClient.videoUpscaler;
